...
namespace ts {
    ...
    function resolvedModuleFromResolved({ ts, js }: Resolved, isExternalLibraryImport: boolean): ResolvedModule {
        return { resolvedTsFileName: ts, resolvedJsFileName: js, isExternalLibraryImport };
    }
    ...
    export function resolveModuleName(moduleName: string, containingFile: string, compilerOptions: CompilerOptions, host: ModuleResolutionHost): ResolvedModuleWithFailedLookupLocations {
        ...
        if (traceEnabled) {
            if (result.resolvedModule) {
                trace(host, Diagnostics.Module_name_0_was_successfully_resolved_to_1, moduleName, resolvedPath(result.resolvedModule));
            }
            else {
